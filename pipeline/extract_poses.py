...
def estimate_head_pose(image_path: str) -> Optional[Dict]:
    ...
    # Calculate bounding box for crop data with padding for full head
    ...
    # Add 40% padding to include full head (hair, forehead, chin)
    ...
    # Coordinate system: viewer expects rx=pitch, ry=yaw, rz=roll
    # MediaPipe gives rotation from camera view, need to adjust for viewer coordinate system

    # Extract rotation angles from rotation matrix
    # Using XYZ Euler angle convention (matches viewer)
    sy = np.sqrt(rotation_matrix[0, 0] ** 2 + rotation_matrix[1, 0] ** 2)
    singular = sy < 1e-6

    if not singular:
        # Standard Euler angle extraction
    ...
    # Convert to degrees
    # If angles near ±180, likely coordinate system inverted - flip by subtracting from 180
    pitch_deg_raw = int(np.degrees(pitch))
    yaw_deg_raw = int(np.degrees(yaw))
    roll_deg_raw = int(np.degrees(roll))
    ...
    if abs(roll_deg_raw) > 90:
        roll_deg = 180 - roll_deg_raw if roll_deg_raw > 0 else -180 - roll_deg_raw
    else:
        roll_deg = roll_deg_raw
    ...
    return {
        ...
        "ry": yaw_deg,
        "rz": roll_deg,
        "url": f"./static/pipeline/input/{filename}",
        "cx": cx,
        "cy": cy,
        "cs": cs,
        "w": w,
